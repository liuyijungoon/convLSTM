...
class CLSTM_cell(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features):
        ...
        self.padding=(int((filter_size-1)/2),int((filter_size-1)/2))#in this way the output has the same size
...
for epoch in range(0, max_epoch):
    ...
    for i,data in enumerate(test_loader):
        seq,target = data
        seq,target = Variable(seq).cuda().float(),Variable(target).cuda().float()
        
        seq = seq.unsqueeze(2)
        out=conv_lstm(seq,hidden_state)
        # pdb.set_trace()
        # for t in range(0, num_features):
        #     pdb.set_trace()   
        loss = loss_fn(out[0][0][0].cpu(), target.cpu()) 
        if i%99==0:
